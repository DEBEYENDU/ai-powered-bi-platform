...
class JobMonitor:
    def status(self) -> Dict[str, Any]:
        try:
            ...
            return {"broker": "unreachable", "workers": [], "running": 0,
                    "scheduled": 0, "reserved": 0,
                    "detail": str(exc)[:200]}

    def failed_jobs_note(self) -> Dict[str, Any]:
        return {"note": "Failed/completed history requires a result backend; "
                        "configure CELERY_RESULT_BACKEND=redis for task history."}

    def queues(self) -> List[Dict[str, Any]]:
        status = self.status()
        return [{"name": "celery", "depth": status.get("reserved", 0),

...
class Tracer:
    ...
    def start_trace(self, name: str, attributes: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def start_span(self, name: str, attributes: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        span = {"trace_id": trace_id_ctx.get() or uuid.uuid4().hex,
                "span_id": uuid.uuid4().hex[:16],
                "parent_span_id": parent, "name": name,
                "attributes": attributes or {}, "start": time.time(),
                "end": None, "duration_ms": None}
        trace_id_ctx.set(span["trace_id"])
        span_stack_ctx.set(tuple(list(stack) + [span]))
        return span

    def end_span(self, span: Dict[str, Any], attributes: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        span["end"] = time.time()
    ...
    def trace(self, name: str, attributes: Optional[Dict[str, Any]] = None):  # type: ignore[no-untyped-def]
        tracer = self

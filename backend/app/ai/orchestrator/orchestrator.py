...
class Orchestrator(BaseModel):
    ...
    async def process_query(
        ...
        if intent_detection.needs_human_review:
            return self._build_human_review_response(request_id, intent_detection, time.time() - start_time)
        ...
        execution_results = await self._execute_plan(plan, request_id, user_id, organization_id, context)
        ...
        response_text = await self._generate_response(intent_detection, execution_results, citations)
    ...
    async def _execute_plan(
        ...
        if parallel_groups:
            ...
            for group_name, steps in parallel_groups.items():
                tasks.append(self._execute_step_group(group_name, steps, request_id, user_id, organization_id, context))
            ...
            for step in ordered_steps:
                result = await self._execute_single_step(step, request_id, user_id, organization_id, context)
    ...
    async def _execute_single_step(
        ...
        if tool_def is None:
            return ToolExecutionResult(
                tool_name=step.tool_name, success=False,
        ...
        try:
            ...
            return ToolExecutionResult(
                tool_name=step.tool_name, success=True,
    ...
    def _build_human_review_response(self, request_id: str, intent: IntentDetection, elapsed: float) -> Dict[str, Any]:
        return {
            "request_id": request_id,
    ...
    def _suggest_follow_up(self, intent: IntentDetection, results: List[ToolExecutionResult]) -> List[str]:
        suggestions = [

...
class ReportBuilder:
    ...
    def __init__(
        self,
        analytics_fetcher: Optional[Any] = None,
        dashboard_fetcher: Optional[Any] = None,
    ...
    async def build(
        ...
        for idx, section in enumerate(sorted(sections, key=lambda s: s.get("order", 0))):
            ...
            built.append(BuiltSectionModel(
    ...
    async def _build_section(self, kind: str, section: Dict[str, Any],
                             variables: Dict[str, Any], include_ai: bool,
                             ai_sections: List[str]) -> tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    def _default_ai(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        return {"text": f"AI section '{payload.get('section')}' (assistant unavailable)", "citations": []}

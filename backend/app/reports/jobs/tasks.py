...
def generate_report_task(
    ...
    try:
        return asyncio.run(
            service.generate(
                report_id=report_id, formats=formats or ["pdf"], variables=variables or {}
            )
        )
...
def run_due_schedules_task() -> dict[str, Any]:
    ...
    for sched in due:
        try:
            ...
            result = asyncio.run(
                service.generate(
                    report_id=sched.report_id, formats=["pdf"], variables={}, user_id="scheduler"
                )
            )
            ...
            service.events.publish(
                "report_failed", sched.report_id, "scheduler", details={"error": str(exc)}
            )

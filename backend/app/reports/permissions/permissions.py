...
class PermissionChecker:
    ...
    def grant(self, report_id: str, granted_to: str, role: str = "viewer",
              can_export: bool = True, can_distribute: bool = False) -> Dict[str, object]:
        if role not in ROLE_RANK:
            raise ValueError(f"Unknown role '{role}'")
        grant = {"granted_to": granted_to, "role": role,
                 "can_export": can_export, "can_distribute": can_distribute}
    ...
    def role_of(self, report_id: str, user_id: str, owner_id: Optional[str] = None,
    ...
    def require(self, report_id: str, action: str, user_id: str,
                owner_id: Optional[str] = None, user_roles: Optional[List[str]] = None,
                department: Optional[str] = None) -> str:
        minimum = {"view": "viewer", "edit": "editor", "review": "reviewer",
                   "approve": "editor", "export": "viewer", "distribute": "editor",
                   "share": "editor", "delete": "owner"}.get(action, "viewer")
        ...
        if action == "export":
            grants = [g for g in self._shares.get(report_id, [])
        ...
        if action == "distribute":
            grants = [g for g in self._shares.get(report_id, [])
                      if str(g.get("granted_to")) in [user_id, "org"]]
            if grants and not any(g.get("can_distribute", False) for g in grants) and role != "owner":

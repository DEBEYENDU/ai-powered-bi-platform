...
from app.reports.schemas.report import (
    ReportCreate, ReportGenerateRequest, ScheduleCreate, ShareCreate,
    TemplateApprove, TemplateCreate,
)
...
def create_report(payload: ReportCreate = Body(...),
...
def list_reports(report_type: Optional[str] = Query(None), status: Optional[str] = Query(None),
                 search: str = Query(""), limit: int = Query(20, ge=1, le=100),
                 offset: int = Query(0, ge=0),
                 service: ReportService = Depends(get_service)):
    records = service.repo.list(report_type=report_type, status=status,
...
def search_reports(query: str = Query(""), report_type: Optional[str] = Query(None),
...
async def generate_report(payload: ReportGenerateRequest = Body(...),
                          service: ReportService = Depends(get_service)):
    try:
        return await service.generate(
            ...
            formats=payload.formats, include_ai=payload.include_ai,
            ai_sections=payload.ai_sections, variables=payload.variables)
    except ValueError as exc:
        raise HTTPException(400, str(exc))


@reports_router.post("/preview", summary="Preview report HTML")
def preview_report(definition: Dict[str, Any] = Body(...),
                   service: ReportService = Depends(get_service)):
...
def download_report(report_id: str, format: str = Query("pdf"),
                    version: int = Query(0),
                    service: ReportService = Depends(get_service)):
    record = service.repo.get(report_id)
    if record is None:
    ...
    target = versions[-1] if version == 0 else next(
...
def compare_versions(report_id: str, from_version: int = Query(...),
...
def restore_version(report_id: str, version: int,
                    service: ReportService = Depends(get_service)):
    record = service.repo.get(report_id)
    if record is None:
        raise HTTPException(404, "Report not found")
...
def approve_report(report_id: str, approved_by: str = Body(""),
                   service: ReportService = Depends(get_service)):
...
def share_report(report_id: str, payload: ShareCreate = Body(...),
                 service: ReportService = Depends(get_service)):
    return service.permissions.grant(report_id, payload.granted_to, payload.role,
                                     payload.can_export, payload.can_distribute)


@reports_router.post("/schedules", summary="Schedule report")
def create_schedule(payload: ScheduleCreate = Body(...),
                    service: ReportService = Depends(get_service)):
    sched = service.scheduler.create(
        report_id=str(payload.report_id), frequency=payload.frequency,
        cron_expression=payload.cron_expression, timezone=payload.timezone,
        distribution=payload.distribution, max_retries=payload.max_retries)
    service.events.publish("report_scheduled", str(payload.report_id),
...
def create_template(payload: TemplateCreate = Body(...),
                    service: ReportService = Depends(get_service)):
    tpl = service.templates.create(payload.name, payload.description,
                                   layout={}, sections=[s.dict() for s in payload.sections],
                                   shared=payload.shared)
    return {"id": tpl.template_id, "name": tpl.name, "current_version": 1,
            "approved": False, "shared": tpl.shared}


@reports_router.get("/templates/list", summary="List templates")
def list_templates(service: ReportService = Depends(get_service)):
    return {"data": [{"id": t.template_id, "name": t.name,
                      "current_version": t.current_version, "shared": t.shared}
                     for t in service.templates.list()]}


@reports_router.post("/templates/{template_id}/approve", summary="Approve template")
def approve_template(template_id: str, payload: TemplateApprove = Body(...),
                     service: ReportService = Depends(get_service)):
    ...
    service.events.publish("template_updated", None, payload.approved_by,
...
def report_history(report_id: str, service: ReportService = Depends(get_service)):
    return {"data": [e.dict() for e in service.events.history(report_id)] + service.audit_trail(report_id)}

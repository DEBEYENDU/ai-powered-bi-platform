...
class ReportService:
    ...
    # -- reports --
    def create_report(
        self, data: dict[str, Any], owner_id: str = "", organization_id: str = ""
    ) -> dict[str, Any]:
        record = self.repo.create(
            {**data, "owner_id": owner_id, "organization_id": organization_id}
        )
        self.permissions.grant(
            record["id"], owner_id or "owner", role="owner", can_export=True, can_distribute=True
        )
        self._log("report_created", record["id"], owner_id, organization_id, {})
        self.events.publish(
            "report_generated", record["id"], owner_id, organization_id, {"action": "created"}
        )
        return record
    ...
    async def generate(
        ...
        if report_id:
            ...
            self.permissions.require(
                report_id,
                "export",
                user_id or record.get("owner_id", ""),
        ...
        cache_key = self.cache.key(
            "generate", definition=definition, formats=formats, variables=variables or {}
        )
        ...
        if template_id:
            try:
                layout = self.templates.apply_layout(
                    template_id, {"title": built.title, **(variables or {})}
                )
            except ValueError:
                layout = {}
        ...
        result = {
            "report_id": report_id or built.report_id,
    ...
    def preview(self, definition: dict[str, Any], variables: dict[str, Any] | None = None) -> str:
        stub = {
            "title": definition.get("title", "Preview"),
    ...
    # -- audit --
    def audit_trail(self, report_id: str | None = None) -> list[dict[str, Any]]:
